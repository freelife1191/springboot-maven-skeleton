...
drop table if exists common_detail_code;
drop table if exists common_empty_code;

create table common_code
(
    id          int unsigned auto_increment comment '공통코드 ID'   primary key,
    group_code  varchar(254)                             not null comment '공통그룹코드 (식별을 위한 하위 그룹코드와 동일한 영문단어 또는 이니셜)',
    code        int unsigned                             not null comment '공통코드(공통코드 ID 하위의 단순증가값)',
    ...
    upper_id    int unsigned                             null comment '상위공통코드 ID',
    depth       int unsigned default 0                   not null comment '뎁스',
    `order`     int unsigned default 0                   not null comment '정렬순서',
...
(
    common_code_id     int unsigned                             not null comment '공통코드 ID',
    detail_code        int unsigned                             not null comment '공통상세코드',
...
(
    ...
    common_code_id int unsigned                         null comment '공통 메인 코드 ID',
    detail_code    int unsigned                         null comment '공통 상세 코드',

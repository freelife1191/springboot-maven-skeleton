...
public class SampleFileController {
    ...
    */
    ...
    @ApiOperation(value = "Sample File 업로드 ModelAttribute 리스트",notes = "리스트로 파일과 데이터를 전송할 때", consumes = MediaType.MULTIPART_FORM_DATA_VALUE ) //, produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Content-Type", defaultValue = MediaType.MULTIPART_FORM_DATA_VALUE, paramType = "header")// +","+MediaType.MULTIPART_MIXED_VALUE, paramType = "header")
    })
    @ApiResponses({@ApiResponse(code = 200, message = "업로드 성공시 파일 업로드 파일정보 리턴")})
    ...
    public ResponseEntity<List<Map<String, String>>> listForm(@ApiParam(value = "파일 객체", required = true)
